...
import { Building2, LayoutDashboard, Users, LogOut, ChevronRight, Calendar, BookOpen, Link2, ClipboardCheck, Wrench, Package, HardHat, Settings, CreditCard, type LucideIcon } from 'lucide-react';
import ThemeToggle from '@/components/ui/ThemeToggle';
import MobileSidebarShell from '@/components/ui/MobileSidebarShell';

type NavItem = { href: string; label: string; icon: LucideIcon };

function renderNavGroup(items: NavItem[], pathname: string) {
  ...
  return (
    <aside className="flex h-full w-56 flex-col border-r border-[var(--border-color)] bg-[var(--bg-surface)]">
      ...
      <nav className="flex-1 overflow-y-auto p-2">
        {renderNavGroup(OPERATE_NAV, pathname)}

        <p className="mt-3 mb-1 px-3 text-[10px] font-semibold uppercase tracking-widest text-[var(--text-tertiary)]">Manage</p>
        {renderNavGroup(MANAGE_NAV, pathname)}

        <p className="mt-3 mb-1 px-3 text-[10px] font-semibold uppercase tracking-widest text-[var(--text-tertiary)]">Operations</p>
        {renderNavGroup(OPS_NAV, pathname)}

        <p className="mt-3 mb-1 px-3 text-[10px] font-semibold uppercase tracking-widest text-[var(--text-tertiary)]">Settings</p>
        {renderNavGroup(SETTINGS_NAV, pathname)}
      </nav>
